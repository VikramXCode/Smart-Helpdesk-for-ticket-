...
const KnowledgeBaseGrid = () => {
  ...
  const headerAction = isStaff && (
    <button onClick={() => setShowAdd(true)} className="flex items-center justify-center rounded-lg px-4 py-2 bg-primary hover:bg-primary/90 text-white text-sm font-bold transition-colors shadow-sm">
  ...
  const knowledgeContent = (
    <div className="flex-1 flex flex-col items-center w-full">
      ...
      <div className="w-full bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 pb-12 pt-10 px-4">
          <button onClick={() => navigate(-1)} className="mb-4 ml-4 flex items-center gap-2 text-slate-600 hover:text-primary transition-colors">
  ...
  return (
    <AdminLayout title="Knowledge Base" headerAction={headerAction}>
        <div className="p-8 max-w-7xl mx-auto">
          ...
          {/* Hero search */}
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 mb-8 pb-8 pt-8 px-6">
            <div className="text-center flex flex-col items-center gap-4">
              <h1 className="text-2xl md:text-3xl font-black tracking-tight text-primary dark:text-white">Knowledge Base</h1>
              <p className="text-slate-500 dark:text-slate-400 text-base max-w-2xl">Search for troubleshooting guides, error codes, policy documents, and more.</p>
              <div className="w-full max-w-2xl mt-2 relative group">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400 group-focus-within:text-primary transition-colors">
                  <span className="material-symbols-outlined">search</span>
                </div>
                <input
                  className="block w-full pl-12 pr-4 py-3 rounded-lg border-2 border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 text-primary dark:text-white placeholder-slate-400 focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all shadow-sm text-base"
              ...
              <div className="flex flex-wrap justify-center gap-2 mt-3">
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs sm:text-sm font-medium transition-all ${!category ? 'bg-primary text-white shadow-sm' : 'bg-white dark:bg-slate-800 hover:bg-slate-50 text-slate-500 border border-slate-200 dark:border-slate-700'}`}>
                  {!category && <span className="material-symbols-outlined text-[16px]">check</span>}{' '}All
                </button>
                {categories.map(cat => (
                  <button
                    key={cat}
                    onClick={() => { setCategory(cat === category ? '' : cat); setPage(1); }}
                    className={`flex items-center gap-1 px-3 py-1.5 rounded-full text-xs sm:text-sm font-medium transition-all ${category === cat ? 'bg-primary text-white shadow-sm' : 'bg-white dark:bg-slate-800 hover:bg-slate-50 text-slate-500 border border-slate-200 dark:border-slate-700'}`}>
                    {cat}
                  </button>
                ))}
              </div>
          ...
          <div className="mb-8">
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {articles.map((article) => (
                  <article key={article.id} className="group bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 p-4 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-pointer flex flex-col h-full">
                    <div className="flex items-center justify-between mb-2">
                      <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium border ${categoryColor(article.category)}`}>
                        {article.category || 'General'}
                      </span>
                      <span className="text-xs text-slate-400 flex items-center gap-1">
                        <span className="material-symbols-outlined text-[12px]">visibility</span> {article.view_count || 0}
                      </span>
                    </div>
                    <h3 className={`text-base font-bold text-primary dark:text-white mb-2 ${hoverColor(article.category)} transition-colors`}>{article.title}</h3>
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-slate-100 dark:border-slate-700 mt-auto">
                      <div className="flex items-center gap-2">
                        <div className="size-5 rounded-full bg-primary text-white flex items-center justify-center text-[8px] font-bold">
                          {article.author_name?.charAt(0) || 'A'}
                        </div>
                        <span className="text-xs font-medium text-slate-500">{article.author_name || 'Admin'}</span>
                      </div>
